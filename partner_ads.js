document.addEventListener('DOMContentLoaded', () => {
  async function checkAccess(username) {
    try {
      const res = await fetch('/api/check-user-min', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ username, minDay: 21 })
      });
  ...
  async function handleSubmit(username, options = {}) {
    ...
    if (result.ok) {
      ...
      inputEl.classList.add('input-invalid');
      showGate(result.message || 'Доступ открыт с 21-го дня участия');
    }
  }
  ...
  if (candidate) {
    handleSubmit(candidate, { redirectToStudy: false });
  }
});
